...
public class MatchRecordDialog extends JDialog{
	...
	private SetUpPanel m_setUpPanel;
	
	public MatchRecordDialog(Event event){
		this.event = event;
		
		mainPanel = new JPanel();
		m_setUpPanel = new SetUpPanel(event);
		
		mainPanel.add(m_setUpPanel);
		mainPanel.add(new JSeparator(SwingConstants.HORIZONTAL));
		mainPanel.add(Box.createRigidArea(new Dimension(0,20)));
		mainPanel.add(new JSeparator(SwingConstants.HORIZONTAL));
		
		JLabel autonLabel = new JLabel("Autonomous");
		makeJlabelBold(autonLabel);
		mainPanel.add(autonLabel);
		
		mainPanel.setLayout(new BoxLayout(mainPanel,BoxLayout.Y_AXIS));
		add(mainPanel);
		pack();
	}
	public MatchRecordDialog(MatchRecord record, Event event){
		this.event = event;
	...
	private static void makeJlabelBold(JLabel l){
...
class SetUpPanel extends JPanel {
	...
	public SetUpPanel(Event event){

    	this.teamNums = new ArrayList();
    	this.teamNames = new ArrayList();
    	
    	for( int i = 0; i < event.getTeamList().size(); i++ ){
			teamNums.add(event.getTeamList().get(i).getNumber());
			teamNames.add(event.getTeamList().get(i).getName());
		}
        ...
        matchNum = new JTextField("Match Number",15);
        Methods.clearAndResetText(matchNum,matchNum.getText());
        scoutName = new JTextField("Scout Name",20);
